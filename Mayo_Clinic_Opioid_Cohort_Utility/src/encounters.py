...
def get_epic_outpatient_encounters_for_cohort(patients: pd.DataFrame, inclusion: list=None, split: int=5000)->pd.DataFrame:
    ...
    """
    if inclusion:
        filter_string= filter.get_filter_string(inclusion, "LOWER(APPOINTMENT_DESCRIPTION) LIKE", "OR")
        filter_string=f"{filter_string}  AND SOURCE_SYSTEM_DK='47918'"
    else:
        filter_string=f"SOURCE_SYSTEM_DK='47918'"
...
def get_encounters_for_cohort(patients: pd.DataFrame, inclusion: list=None, split: int=5000)->pd.DataFrame:
    ...
    """
    if inclusion:
        filter_string= filter.get_filter_string(inclusion, "LOWER(ENCOUNTER_TYPE) LIKE", "OR")
    else:
        filter_string=None
